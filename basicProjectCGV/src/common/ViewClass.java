...
public class ViewClass {
	...
	private String inputRegno2() {
		...
		while (true) {
			System.out.println();
			System.out.println("┌――――――――――――――――――――┐");
			System.out.println("\t주민등록번호 뒷자리를 입력해주세요");
			System.out.println("└―――――――――━――――――――――┘");
	...
	private void pageWriteReview() {
		while (true) {
			try {
				...
				System.out.println();
				Scanner sc = new Scanner(System.in);
				System.out.print("리뷰를 작성할 영화 번호를 선택하세요 : ");
				int select = sc.nextInt();
		...
		while (true) {
			try {
				...
				if (select > 0 && select < movieNameList.size() + 1) {
					if (reviewSv.selectMovieReview(movieNameList.get(select)) == false) { 
