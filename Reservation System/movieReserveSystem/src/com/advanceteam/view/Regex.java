...
/**
 * MemverVO
 * mem_id, mem_pw, mem_hp, mem_regno1, mem_regno2, mem_name를 입력할 때
 * 해당 변수마다 요구하는 패턴에 일치하는지를 검사한다.
 * @author 신광진
 * 
 */
public class Regex {
	...
	public boolean checkHp(String mem_hp) {
		...
		//'-'을 제외하고 핸드폰 번호를 입력한 경우
		String regex_hp2 = "^01(?:0|1|[6-9])(?:\\d{3}|\\d{4})\\d{4}$";
		
		if(Pattern.matches(regex_hp, mem_hp) || Pattern.matches(regex_hp2, mem_hp)) {
			return true;
		}
	...
	public boolean checkReview(String review) {
		...
		String regex_review = "^[가-힣\\s]{1,50}$";
